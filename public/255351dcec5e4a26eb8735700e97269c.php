<?php /*a:1:{s:23:"../view/grade-list.html";i:1577437782;}*/ ?>
<!DOCTYPE html>
        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                ...
                <div class="layui-card-header">
                    ...
                    <button class="layui-btn" onclick="xadmin.open('添加记录','grade-add',430,480)"><i class="layui-icon"></i>添加记录</button>
                    <button class="layui-btn layui-btn-normal" onclick="window.location.href='grade-list'">显示所有</button>
                </div>
                <div class="layui-card-body layui-table-body layui-table-main">
                    <table class="layui-table layui-form">
                        ...
                        <tr>
                            ...
                            <td><?php echo htmlentities($key['SUBJECT']); ?></td>
                            <td><?php echo htmlentities($key['GRADE']); ?></td>
                            <td><?php echo htmlentities($key['STATUS']); ?></td>
                            <td class="td-manage">
                                <a title="编辑"  onclick="xadmin.open('编辑','grade-edit?id=<?php echo htmlentities($key['ID']); ?>',430,550);" href="javascript:;">
                    ...
                    <div class="page">
                        <div>
                            <form class="layui-form">
                                ...
                                <?php if($id>0): ?>
                                <a class="prev" href="grade-list?id=<?php echo htmlentities($id-1); ?>">上一页</a>
                                <?php endif; 
                                if($page>7){
                                for($i=($id-3);$i<=$id;$i++){
                                if($i>=1){
                                ...
                                echo '<a class="num" href="grade-list?id='.($i-1).'">'.$i.'</a>';
                                }
                                }
                                for($i=($id+1);$i<=($id+5);$i++){
                                if($i<=$page){
                                ...
                                echo '<a class="num" href="grade-list?id='.($i-1).'">'.$i.'</a>';
                                 ...
                                 if(($id+1)!=$page): ?>
                                <a class="next" href="grade-list?id=<?php echo htmlentities($id+1); ?>">下一页</a>
                                <?php endif; ?>
                                <div class="layui-inline layui-show-xs-block">
        ...
        function() {
            ...
            form.on('submit(sreach)',
                function(data) {
                    ...
                    window.location.href="grade-list?choose="+data.field.optioin+"&message="+data.field.message;
                    return false;
                });
            //跳转页面
            form.on('submit(goto)',
                function(data) {
                    window.location.href="grade-list?id="+(data.field.message-1);
                    return false;
                });
        });
    ...
    function delAll (argument) {
        ...
        layer.confirm('确认要删除吗？',function(index){
            ...
            del=$.ajax({
                type: 'post',
                url:'/tools/changmdel',
                data:{ids:ids.toString(),type:'grade'},
                success:function (msg) {
